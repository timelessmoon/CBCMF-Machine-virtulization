...
class MTCAgent:
    # @function: init
    def __init__(self, config, metadata, errorTemplate, hostName, sequenceBufferSize=10, sampleRate=1):
        # Instance variables
        self.devices = {} #{"deviceName":Uarm} # Dictionary of devices {"deviceName":deviceObject"}
        self.sequences = {}
        self.sequenceBufferSize = sequenceBufferSize #10
        self.sampleRate = sampleRate #1
        # The sequences.
        ...
        self.loadDeviceFromConfig(config, metadata, hostName)
    ...
    def loadDeviceFromConfig(self, configFilePath, metadataFilePath, hostName):
        ...
        # Parse the xml document for metadata
        self.metadata = ElementTree.parse(metadataFilePath)
    ...
    def metadataEncode(self):
        # Grab the string version of the element tree.
        eTreeString = ElementTree.tostring(self.metadata.getroot())
        # Remove the "ns0" that seems to be forced by the element tree object.
        # eTreeString = eTreeString.replace(":ns0", "")
        # eTreeString = eTreeString.replace("ns0:", "")
        return eTreeString
...
agent = MTCAgent(config["DeviceConfigurationFile"], config["DeviceMetadataConfigurationFile"], config["ErrorReference"], HOST_NAME)
    ...
    def do_GET(s):
        ...
        try:
            ...
            # end if probe
                ...
                # Write the data.
                s.wfile.write(agent.metadataEncode())
            # end if metadata
